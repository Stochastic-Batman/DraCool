...
\title{Math behind \texttt{TenShadows}}
...
\begin{remark}
The word \emph{altitude} is overloaded and the collision is worth flagging. In aviation and in most GIS contexts, altitude means height above sea level, measured in meters. In astronomy, and throughout this document, altitude means an \emph{angle} above the horizon. \texttt{TenShadows} uses the astronomical sense exclusively. Building heights are always written $h$ and always in meters.
\end{remark}
...
Computing $(a, A)$ from a timestamp and a location is a solved problem in astronomy involving the Earth's orbital elements, its axial tilt, and the equation of time. \texttt{TenShadows} does not re-derive it and this document will not either; I treat it as an oracle
...
\texttt{TenShadows} deliberately uses \texttt{suncalc-py} in the Python pipeline and \texttt{suncalc} in the browser, rather than a different library on each side. The reason is not accuracy but \emph{convention}. SunCalc reports azimuth in radians measured clockwise \emph{from south}, whereas the other Python astronomy library I have heard of (\texttt{pvlib}) reports degrees measured clockwise \emph{from north}. Mixing the two conventions rotates every shadow in the city by exactly $180^\circ$, which is obviously not something we want. Where SunCalc's south-based angle $A_{\mathrm{sc}}$ must be converted to the north-based $A$ used throughout this document, the conversion is
